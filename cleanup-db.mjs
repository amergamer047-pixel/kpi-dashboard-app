...
import { departments, categories, indicators } from "./drizzle/schema.ts";
...
async function cleanup() {
  try {
    ...
    for (const ind of testInds) {
      ...
      await db.delete(indicators).where(eq(indicators.id, ind.id));
    }
    
    // Show remaining data
    const finalDepts = await db.select().from(departments);
    const finalCats = await db.select().from(categories);
    const finalInds = await db.select().from(indicators);
